...
def print_stats(stats, phase):
    if 'num_pairs' in stats:
        ...
        s = '{} - Mean loss: {:.6f}    Avg. embedding norm: {:.4f}   Pairs per batch (all/non-zero pos/non-zero neg): {:.1f}'
...
def do_train(dataloaders, params: MinkLocParams, debug=False, visualize=False, resume=None):
    ...
    model_pathname = os.path.join(weights_path, 'log', model_name)
    #if not os.path.exists(model_pathname):
    #    os.mkdir(model_pathname)
    #model_pathname = os.path.join(weights_path, 'log', model_name, model_name)
    ...
    if torch.cuda.is_available():
        ...
        if resume is not None:
            assert os.path.exists(resume), 'Cannot open network weights: {}'.format(resume)
            print('Loading weights: {}'.format(resume))
            model.load_state_dict(torch.load(resume, map_location=device))
            model_pathname += '_resume'
        model.to(device)
    else:
        device = "cpu"
    ...
    if params.scheduler is None:
        ...
        if params.scheduler == 'CosineAnnealingLR':
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,
                                                                   T_max=params.epochs+1,
                                                                   eta_min=params.min_lr)
        elif params.scheduler == 'MultiStepLR':
            scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                             params.scheduler_milestones,
                                                             gamma=0.1)
        else:
    ...
    for epoch in tqdm.tqdm(range(1, params.epochs + 1)):
        ...
        if epoch == 1:
            if not os.path.exists(model_pathname):
                os.mkdir(model_pathname)
        final_model_path = os.path.join(model_pathname, f'epoch_{epoch}.pth')
